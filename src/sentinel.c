...
typedef struct sentinelRedisInstance {
    ...
    mstime_t last_pub_time;   /* Last time we sent hello via Pub/Sub. */                    //上一次发送hello的时间
    mstime_t last_hello_time; /* Only used if SRI_SENTINEL is set. Last time                //上一次收到hello的时间，SRI_SENTINEL设置才有效
...
struct sentinelState {
    ...
    int deny_scripts_reconfig; /* Allow SENTINEL SET ... to change script               //运行时是否可以修改脚本
                                  paths at runtime? */
    char *sentinel_auth_pass;    /* Password to use for AUTH against other sentinel */  //密码
    char *sentinel_auth_user;    /* Username for ACLs AUTH against other sentinel. */   //用户
    int resolve_hostnames;       /* Support use of hostnames, assuming DNS is well configured. */   //是否允许使用host，而不是ip
    int announce_hostnames;      /* Announce hostnames instead of IPs when we have them. */         //向其他哨兵实例发送的host信息
} sentinel;
...
void initSentinel(void) {
    ...
    //清空命令列表
    //替换为哨兵的命令
    //要注意哨兵模式下，很多命令与server模式下，命令相同，函数却不同
...
NULL
               ...
               !strcasecmp(c->argv[1]->ptr,"replicas"))
    {
        ...
        //如果本服务没有处于tilt状态
        //被查询是主服务，而且状态为SRI_S_DOWN【此哨兵也认为主服务已经下线】，则返回服务已经下线消息
...
void sentinelCheckSubjectivelyDown(sentinelRedisInstance *ri) {
    ...
    {
        //条件一：当前距离上次发送 PING 的时长已经超过 down_after_period 阈值，还没有收到回复，默认值是 30s。
...
void sentinelCheckObjectivelyDown(sentinelRedisInstance *master) {
    ...
    //如果判断主节点sdown的数量超过了master->quorum，odown就发生了
...
void sentinelAskMasterStateToOtherSentinels(sentinelRedisInstance *master, int flags) {
    ...
    while((de = dictNext(di)) != NULL) {
        ...
        //依次向其他节点发送is-master-down-by-addr
        //哨兵会根据当前主节点所处的状态来设置实例ID
        //如果主节点已经要开始进行故障切换了，实例ID会被设置为当前哨兵自身的ID，否则就会被设置为*号。
        /* Ask */
...
char *sentinelGetLeader(sentinelRedisInstance *master, uint64_t epoch) {
    ...
    //当有一个节点获得超过半数的其他哨兵的赞成票，同时获得超过预设的 quorum 阈值的赞成票数
...
void sentinelFailoverWaitStart(sentinelRedisInstance *ri) {
    ...
    if (!isleader && !(ri->flags & SRI_FORCE_FAILOVER)) {
        ...
        //如果自己不是主节点，而且没有被要求SRI_FORCE_FAILOVER
        //而且一段时间后也没有成为主节点，退出故障转移状态
    ...
    //进入SENTINEL_FAILOVER_STATE_SELECT_SLAVE，开始查找合适的slave
...
//故障转移状态机
//刚开始故障转移时，状态为SENTINEL_FAILOVER_STATE_WAIT_START
//“+failover-triggered”: leader开始进行failover
//“+failover-state-wait-start”，选出leader
//“+failover-state-select-slave”: leader开始查找合适的slave
//“+selected-slave”: 已经找到合适的slave
//“+failover-state-sen-slaveof-noone”: leader向slave发送“slaveof no one”指令，升级为master
...
//“+slave-reconf-done”: 此slave同步完成，此后leader可以继续下一个slave的reconfig操作。循环G）
//“+failover-end”: 故障转移结束
...
void sentinelHandleRedisInstance(sentinelRedisInstance *ri) {
    ...
    //尝试与断掉的实例重新建立连接，包括命令连接以及订阅发布连接
    sentinelReconnectInstance(ri);

    //发送info消息，ping消息，发送hello消息
    sentinelSendPeriodicCommands(ri);

    //tilt时，哨兵只会定期收集信息，而不会执行故障切换流程。
    /* ============== ACTING HALF ============= */
...
void sentinelHandleDictOfRedisInstances(dict *instances) {
    ...
    while((de = dictNext(di)) != NULL) {
        ...
        //依次调用sentinelHandleRedisInstance处理实例
        sentinelHandleRedisInstance(ri);

        //如果是主节点，还要处理其全部的slaves、哨兵
        if (ri->flags & SRI_MASTER) {
...
void sentinelTimer(void) {
    //当哨兵连续两次的时间事件处理间隔时长为负值，或是间隔时长过长，那么哨兵就会进入 TILT 模式。
    //在该模式下，哨兵只会定期发送命令收集信息，而不会执行故障切换流程。
    sentinelCheckTiltCondition();
    
    //订阅hello频道路径
    //sentinelTimer->sentinelHandleDictOfRedisInstances->sentinelHandleRedisInstance->
    //sentinelReconnectInstance->redisAsyncCommand
    //消息处理函数为sentinelReceiveHelloMessages
    //
    //发布信息路径
    //sentinelTimer->sentinelHandleDictOfRedisInstances->sentinelHandleRedisInstance->
    ...
    //随机修改sentinelTimer执行频率，防止同时开启的实例，同时发起投票
